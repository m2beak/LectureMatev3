...
export const FlashcardStudy = ({ note, onClose }: FlashcardStudyProps) => {
  ...
  const generateFlashcards = async () => {
    // Requirements:
    // 1. Generate questions based on the User's Notes, NOT the video.
    const noteContent = note.content.trim();

    // We can also include timestamps descriptions if available, as that counts as "user notes".
    // But to be super strict with "User's Notes", let's prioritize the main content.
    // If main content is empty, we check timestamps. 
    // If both empty -> Toast.

    // Wait, the requirement says "content (text) of the current note".

    let textForAI = noteContent;

    // Optional: Include timestamp labels as they are user-generated notes too.
    if (note.timestamps.length > 0) {
      const timestampNotes = note.timestamps.map(t => t.label).join("\n");
      textForAI = `${textForAI}\n\nTimestamp Notes:\n${timestampNotes}`;
    }

    if (!textForAI.trim()) {
      toast.error("Write some notes first!", {
        description: "Add some notes to generate flashcards.",
      });
      onClose();
      return;
    }
  ...
  if (session.cards.length === 0 && !isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-center gap-4">
        ...
        <Button
          ...
          size="icon"
          onClick={() => {
            // Allow skipping if not the last card or if last card but not answered yet (though that's weird)
            // Actually just sticking to standard prev/next for navigation.
            if (session.currentIndex < session.cards.length - 1) {
              handleNext();
            }
          }}
